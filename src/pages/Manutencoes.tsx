...
import { AlertTriangle, ClipboardList, Loader2, PackageOpen, Plus, Printer, Search, Wrench } from "lucide-react";
...
export default function Manutencoes() {
  const { role, empresaId: companyId, empresaReadOnly: readOnly, empresaNome, isMasterAdmin } = useAuth();
  ...
  const printMaintenanceReport = async () => {
    if (!companyId || isPrinting) return;
    ...
    try {
      await printHtmlDocument({
        companyId,
        purpose: "documento",
        documentName: "Relatório de manutenção",
        widthMm: 210,
        heightMm: 297,
        html: buildA4DocumentHtml({
          companyName: empresaNome ?? "Backstage Pro",
          title: "Relatório de manutenção de equipamentos",
          subtitle: `${state.orders.total} ordem(ns), conforme os filtros atuais · página ${page} de ${pages}`,
          metrics: [
            { label: "Ordens abertas", value: state.indicators.abertas },
            { label: "Em manutenção", value: state.indicators.em_manutencao },
            { label: "Aguardando peça", value: state.indicators.aguardando_peca },
            { label: "Atrasadas", value: state.indicators.atrasadas },
          ],
          sections: [
            {
              title: "Ordens exibidas nesta página",
              columns: [
                "Número", "Material", "Prioridade", "Tipo", "Status",
                "Responsável", "Abertura", "Previsão", "Custo",
              ],
              rows: state.orders.items.map((order) => [
                order.numero,
                `${order.material_nome} (${order.material_codigo})`,
                MAINTENANCE_PRIORITY_LABELS[order.prioridade],
                MAINTENANCE_TYPE_LABELS[order.tipo],
                `${order.atrasada ? "Atrasada · " : ""}${MAINTENANCE_STATUS_LABELS[order.status]}`,
                order.responsavel_nome ?? "A definir",
                new Date(order.aberta_em).toLocaleDateString("pt-BR"),
                order.previsao_conclusao_em
                  ? new Date(order.previsao_conclusao_em).toLocaleDateString("pt-BR")
                  : "—",
                money.format(Number(order.custo_total)),
              ]),
            },
          ],
        }),
  ...
  return <div className="space-y-5">
    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><h1 className="text-2xl font-bold">Manutenção de Equipamentos</h1><p className="text-muted-foreground">Preventivas, corretivas, inspeções e indisponibilidade operacional.</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" disabled={isPrinting || state.isLoading} onClick={printMaintenanceReport}><Printer className="mr-2 h-4 w-4" />{isPrinting ? "Preparando..." : "Imprimir"}</Button>{permissions.criar && <Button onClick={() => setNewOpen(true)}><Plus className="mr-2 h-4 w-4" />Nova ordem</Button>}</div></div>
    ...
    <Card><CardHeader><CardTitle>Ordens de manutenção</CardTitle></CardHeader><CardContent className="space-y-4">
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><div className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><Input className="pl-9" value={filters.search} onChange={(event) => setFilters((current) => ({ ...current, search: event.target.value }))} placeholder="Número, material, QR ou serial" /></div>
        ...
        <Input value={filters.responsible} onChange={(event) => setFilters((current) => ({ ...current, responsible: event.target.value }))} placeholder="Responsável" /><DatePicker value={filters.dateFrom} onChange={(value) => setFilters((current) => ({ ...current, dateFrom: value }))} clearable placeholder="De" /><DatePicker value={filters.dateTo} onChange={(value) => setFilters((current) => ({ ...current, dateTo: value }))} clearable placeholder="Até" /><Button variant="ghost" onClick={() => setFilters(initialFilters)}>Limpar filtros</Button>
      </div>
