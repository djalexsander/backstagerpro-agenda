  ...
  const { data: financials = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("financials")
        .select("*, events(name, artist, date)")
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div><p className="text-lg font-bold">{fmt(totalCache)}</p><p className="text-xs text-muted-foreground">Total Cachê</p></div>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div><p className="text-lg font-bold">{fmt(totalCosts)}</p><p className="text-xs text-muted-foreground">Total Custos</p></div>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div><p className={`text-lg font-bold ${totalProfit >= 0 ? "text-accent" : "text-destructive"}`}>{fmt(totalProfit)}</p><p className="text-xs text-muted-foreground">Resultado</p></div>
            </div>
          </CardContent>
        </Card>
      </div>
