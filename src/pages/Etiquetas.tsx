...
export default function Etiquetas() {
  ...
  const [historyDetail, setHistoryDetail] = useState<LabelPrintBatch | null>(null);
  ...
  return <div className="space-y-5">
    ...
    <Card><CardHeader><CardTitle>Montar lote de etiquetas</CardTitle></CardHeader><CardContent className="space-y-4">
      ...
      <p className="text-xs text-muted-foreground">A escala e margens físicas finais dependem do navegador, driver e impressora.</p>
    </CardContent></Card>
    <Card><CardHeader><CardTitle>Histórico de solicitações</CardTitle></CardHeader><CardContent><div className={`overflow-x-auto rounded-md border ${history.isFetching ? "opacity-60" : ""}`}><Table><TableHeader><TableRow><TableHead>Data</TableHead><TableHead>Materiais / snapshots</TableHead><TableHead>Modelo / versão</TableHead><TableHead>Etiquetas</TableHead><TableHead>Solicitante</TableHead><TableHead /></TableRow></TableHeader><TableBody>{(history.data?.items ?? []).map((item) => <TableRow key={item.id}><TableCell>{new Date(item.solicitada_em).toLocaleString("pt-BR")}</TableCell><TableCell><strong>{item.quantidade_materiais} material(is)</strong>{item.itens.map((entry) => <p key={entry.id} className="text-xs text-muted-foreground">{entry.material_snapshot.nome} ({entry.material_snapshot.codigo_interno}) × {entry.quantidade}</p>)}</TableCell><TableCell>{item.modelo_snapshot.nome} · v{item.modelo_snapshot.versao}</TableCell><TableCell>{item.quantidade_etiquetas}</TableCell><TableCell>{item.solicitante_nome}</TableCell><TableCell className="text-right"><Button variant="outline" size="sm" onClick={() => setHistoryDetail(item)}>Detalhes</Button></TableCell></TableRow>)}{!history.isLoading && !(history.data?.items ?? []).length && <TableRow><TableCell colSpan={6} className="h-24 text-center text-muted-foreground">Nenhuma solicitação registrada.</TableCell></TableRow>}</TableBody></Table></div><div className="mt-3 flex items-center justify-between gap-3"><p className="text-xs text-muted-foreground">Página {historyPage} de {pagination.totalPages}{history.data?.total ? ` · ${history.data.total} solicitação(ões)` : ""}</p><div className="flex gap-2"><Button variant="outline" size="sm" disabled={!pagination.canGoPrevious || history.isFetching} onClick={() => setHistoryPage((page) => Math.max(1, page - 1))}><ChevronLeft className="mr-1 h-4 w-4" />Anterior</Button><Button variant="outline" size="sm" disabled={history.isFetching || !pagination.canGoNext} onClick={() => setHistoryPage((page) => page + 1)}>Próxima<ChevronRight className="ml-1 h-4 w-4" /></Button></div></div></CardContent></Card>
    {companyId && <><LabelModelDialog open={modelDialogOpen} onOpenChange={setModelDialogOpen} companyId={companyId} model={editingModel} onSaved={async () => { await invalidate(); }} /><LabelPrintDialog open={printOpen} onOpenChange={setPrintOpen} companyId={companyId} companyName={companyName} model={selectedModel} items={batchItems} onPrinted={async () => { await invalidate(); setBatchItems([]); }} /></>}
    <Dialog open={Boolean(historyDetail)} onOpenChange={(open) => { if (!open) setHistoryDetail(null); }}><DialogContent className="max-h-[85vh] max-w-3xl overflow-y-auto"><DialogHeader><DialogTitle>Detalhes da solicitação</DialogTitle></DialogHeader>{historyDetail && <div className="space-y-3"><p className="text-sm"><strong>{historyDetail.modelo_snapshot.nome}</strong> · {historyDetail.quantidade_materiais} material(is) · {historyDetail.quantidade_etiquetas} etiqueta(s)</p>{historyDetail.itens.map((item) => <div key={item.id} className="rounded-md border p-3"><div className="flex justify-between gap-3"><strong>{item.material_snapshot.nome}</strong><Badge variant="secondary">{item.quantidade} etiqueta(s)</Badge></div><dl className="mt-2 grid gap-x-4 gap-y-1 text-sm sm:grid-cols-2"><div><dt className="text-muted-foreground">Código interno</dt><dd>{item.material_snapshot.codigo_interno}</dd></div><div><dt className="text-muted-foreground">Identificador canônico</dt><dd className="break-all font-mono text-xs">{item.material_snapshot.identificador_unico}</dd></div><div><dt className="text-muted-foreground">Patrimônio</dt><dd>{item.material_snapshot.numero_patrimonio ?? "—"}</dd></div><div><dt className="text-muted-foreground">Código de barras</dt><dd className="break-all font-mono text-xs">{item.material_snapshot.codigo_barras ?? "—"}</dd></div></dl></div>)}</div>}</DialogContent></Dialog>
  </div>;
}
