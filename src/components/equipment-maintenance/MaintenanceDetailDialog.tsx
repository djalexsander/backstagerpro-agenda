...
export function MaintenanceDetailDialog({ open, onOpenChange, companyId, orderId, responsibles, canWrite, onChanged }: {
  ...
  const save = () => { if (!order) return; mutation.mutate(() => updateMaintenanceOrder(companyId, order, { priority, diagnosis, service,
    responsible: selectedResponsible, expectedCompletion: expected, exitCondition, notes, execution, externalSupplier: supplier,
    preventiveIntervalDays: interval ? Number(interval) : undefined, laborCost: labor, partsCost: Number(order.custo_pecas), otherCost: other, clientUuid: crypto.randomUUID() })); };
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[94vh] max-w-6xl overflow-y-auto"><DialogHeader>
    ...
    {order && <div className="space-y-4">
      ...
      {canWrite && actions && <div className="flex flex-wrap gap-2">{actions.nextStatuses.filter((status) => status !== "cancelada" && (status !== "concluida" || actions.canConclude)).map((status) => <Button key={status} variant="outline" disabled={mutation.isPending} onClick={() => transition(status)}>{MAINTENANCE_STATUS_LABELS[status]}</Button>)}</div>}
      <Tabs defaultValue="ordem"><TabsList><TabsTrigger value="ordem">Ordem</TabsTrigger><TabsTrigger value="insumos">Peças e insumos</TabsTrigger><TabsTrigger value="historico">Histórico</TabsTrigger></TabsList>
        ...
        </div>{canWrite && actions?.canEdit && <Button disabled={mutation.isPending} onClick={save}>Salvar dados</Button>}
