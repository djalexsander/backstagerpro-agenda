...
export function FinanceCards({ financials, getCachePago, getCachePendente, onTogglePayment }: FinanceCardProps) {
  ...
  // Gather all pending items across all financials
  ...
  const handleTogglePaid = (item: typeof pendingItems[0]) => {
    ...
    if (item.type === "entrada") {
      newDetail.entradaPaga = true;
    } else if (item.type === "parcela" && item.parcelaIndex !== undefined) {
      newDetail.parcelas[item.parcelaIndex].pago = true;
    } else if (item.type === "recebimento") {
      newDetail.recebimentoPago = true;
    }
    onTogglePayment(item.financialId, newDetail);
  };

  // Detail breakdowns per event for costs
  const costBreakdown = financials.map((f) => {
    ...
    return {
      ...
      total: (f.transport || 0) + (f.food || 0) + (f.lodging || 0) + (f.other_costs || 0) + sumExtraCosts(extras),
    };
  });
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
      ...
      <div className="space-y-0">
        <Card className="cursor-pointer hover:ring-1 hover:ring-primary/30 transition-all" onClick={() => toggle("cache")}>
          <CardContent className="pt-4 pb-3 px-3">
            <div className="flex items-center gap-2">
              ...
              {expanded === "cache" ? <ChevronUp className="h-3 w-3 text-muted-foreground shrink-0" /> : <ChevronDown className="h-3 w-3 text-muted-foreground shrink-0" />}
            </div>
          </CardContent>
        </Card>
        {expanded === "cache" && (
          <div className="rounded-b-lg border border-t-0 bg-card p-3 space-y-2 text-xs">
            {financials.map((f) => (
      ...
      {/* Recebido */}
      <div className="space-y-0">
        <Card className="cursor-pointer hover:ring-1 hover:ring-accent/30 transition-all" onClick={() => toggle("recebido")}>
          <CardContent className="pt-4 pb-3 px-3">
            <div className="flex items-center gap-2">
              <div className="h-8 w-8 shrink-0 rounded-lg bg-accent/10 flex items-center justify-center">
              ...
              {expanded === "recebido" ? <ChevronUp className="h-3 w-3 text-muted-foreground shrink-0" /> : <ChevronDown className="h-3 w-3 text-muted-foreground shrink-0" />}
            </div>
          </CardContent>
        </Card>
        {expanded === "recebido" && (
      ...
      {/* Pendente */}
      <div className="space-y-0">
        <Card className="cursor-pointer hover:ring-1 hover:ring-[hsl(var(--warning))]/30 transition-all" onClick={() => toggle("pendente")}>
          <CardContent className="pt-4 pb-3 px-3">
            <div className="flex items-center gap-2">
              ...
              {expanded === "pendente" ? <ChevronUp className="h-3 w-3 text-muted-foreground shrink-0" /> : <ChevronDown className="h-3 w-3 text-muted-foreground shrink-0" />}
            </div>
          </CardContent>
        </Card>
        {expanded === "pendente" && (
      ...
      {/* Total Custos */}
      <div className="space-y-0">
        <Card className="cursor-pointer hover:ring-1 hover:ring-destructive/30 transition-all" onClick={() => toggle("custos")}>
          <CardContent className="pt-4 pb-3 px-3">
            <div className="flex items-center gap-2">
              ...
              {expanded === "custos" ? <ChevronUp className="h-3 w-3 text-muted-foreground shrink-0" /> : <ChevronDown className="h-3 w-3 text-muted-foreground shrink-0" />}
            </div>
          </CardContent>
        </Card>
        {expanded === "custos" && (
      ...
      {/* Resultado */}
      <div className="space-y-0">
        <Card className="cursor-pointer hover:ring-1 hover:ring-primary/30 transition-all" onClick={() => toggle("resultado")}>
          <CardContent className="pt-4 pb-3 px-3">
            <div className="flex items-center gap-2">
              ...
              {expanded === "resultado" ? <ChevronUp className="h-3 w-3 text-muted-foreground shrink-0" /> : <ChevronDown className="h-3 w-3 text-muted-foreground shrink-0" />}
            </div>
          </CardContent>
        </Card>
        {expanded === "resultado" && (
