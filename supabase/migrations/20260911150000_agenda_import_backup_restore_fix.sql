-- P1-10 follow-up: restore_company_backup drops the 6 agenda-import columns
-- added to public.events by 20260827120000_events_import_fields_and_days.sql
-- (state, setup_time, staff_notes, contratante_nome, contratante_cidade,
-- contratante_telefone).
--
-- gather_company_backup_data already captures them correctly - its 'eventos'
-- collection uses to_jsonb(t) over the whole row (20260818150000, further
-- back to 20260817200000), which automatically picks up every column
-- public.events has at read time, these six included. The bug is entirely
-- on the write side: restore_company_backup's `INSERT INTO public.events
-- (...)` names an explicit column list written before 20260827120000
-- existed, so jsonb_populate_recordset happily parses the six fields out of
-- the backup JSON into the populated record, and then the INSERT's column
-- list just never reads them back out - a silent data-loss bug on restore,
-- not a capture bug.
--
-- Fix: add the six columns to both the INSERT column list and the SELECT
-- list (sourced from the same `event` record jsonb_populate_recordset
-- already produces). Backward compatible with any backup taken before
-- 20260827120000 by construction: those backups' 'eventos' JSON objects
-- simply have no `state`/`setup_time`/... keys, and
-- jsonb_populate_recordset(NULL::public.events, v_events) already leaves an
-- output column NULL whenever its JSON key is absent - no COALESCE or
-- version branching needed since all six columns are nullable. Every other
-- table/branch in this function (funcionarios, clientes, financials, ...)
-- is untouched; only the public.events INSERT statement changes.
--
-- Old-imported-data investigation (see ETAPA 6 report): checked whether any
-- already-imported event on zupcxxtnaglcappazciu has data trapped in
-- observations instead of these columns, as a prior audit speculated might
-- have happened before this fix existed. Read both versions of
-- importar_agenda_eventos (20260827110000 and 20260827120000) and the sole,
-- never-revised commit of src/lib/agenda-import.ts (f6d8abd): neither the
-- original RPC nor the frontend parser ever concatenated
-- state/setup_time/staff_notes/contratante_* into observations - observations
-- has only ever been the source's own `notes` field, plus one optional
-- fixed line for a date-only logistics departure. Directly queried the 5
-- real event_import_origins rows that exist on the remote (2 companies,
-- imported 2026-08-27/28): all of them already have state/contratante_*
-- correctly in their own columns and observations = NULL. There is no
-- affected data to repair, and - because the old format never existed as a
-- concatenation scheme - there would be no deterministic way to reconstruct
-- one even if it did. See diagnose_imported_events_missing_fields() below:
-- a permanent, read-only tool for any future case, rather than a one-time
-- guess.
...
-- ============================================================================
-- diagnose_imported_events_missing_fields - read-only manual-review tool
-- ============================================================================
--
-- Surfaces, without ever writing anything, imported events (present in
-- event_import_origins) whose six agenda-import columns are ALL null while
-- observations is non-blank - the shape a pre-20260827120000 import (or a
-- restore from a backup taken before that migration and before this same
-- migration's restore fix) would have left behind, IF that had ever
-- happened. As of this migration, zero rows match on the linked remote
-- (verified directly) - see the header comment above for how that was
-- confirmed rather than assumed.
--
-- Deliberately does NOT attempt to parse observations back into
-- state/setup_time/staff_notes/contratante_* - there was never a
-- concatenation scheme to reverse (observations only ever held the
-- source's own free-text notes plus one fixed date-only-departure line),
-- so any such split would be guessing at free text, not a deterministic
-- repair. A flagged row needs a human to read its observations and decide,
-- per the specific case, whether anything belongs in the dedicated columns.
CREATE OR REPLACE FUNCTION public.diagnose_imported_events_missing_fields(
...
BEGIN
  ...
  v_company_id := public.get_user_empresa_id(v_actor_id);
  IF v_company_id IS NULL OR (_empresa_id IS NOT NULL AND _empresa_id IS DISTINCT FROM v_company_id) THEN
    RAISE EXCEPTION USING ERRCODE = '42501',
      MESSAGE = 'Esta consulta só pode ser feita para a empresa do usuário autenticado.';
  END IF;

  -- Same bar as gather_company_backup_data: an administrator of the
  -- company, or master. Read-only diagnostic, so no operational-access gate.
  IF NOT (
    public.has_role(v_actor_id, 'admin_empresa'::public.app_role)
    OR public.is_master_admin(v_actor_id)
  ) THEN
    RAISE EXCEPTION USING ERRCODE = '42501',
      MESSAGE = 'Somente um administrador da empresa pode revisar eventos importados.';
  END IF;

  RETURN QUERY
  SELECT
    e.id, e.name, e.date, o.source_system, o.source_event_id, o.imported_at, e.observations
  FROM public.events AS e
  JOIN public.event_import_origins AS o ON o.event_id = e.id
  WHERE e.empresa_id = v_company_id
    AND e.state IS NULL
    AND e.setup_time IS NULL
    AND e.staff_notes IS NULL
    AND e.contratante_nome IS NULL
    AND e.contratante_cidade IS NULL
    AND e.contratante_telefone IS NULL
    AND nullif(btrim(e.observations), '') IS NOT NULL
  ORDER BY o.imported_at;
END;
$$;

REVOKE ALL ON FUNCTION public.diagnose_imported_events_missing_fields(uuid) FROM PUBLIC, anon, service_role;
GRANT EXECUTE ON FUNCTION public.diagnose_imported_events_missing_fields(uuid) TO authenticated;

COMMENT ON FUNCTION public.diagnose_imported_events_missing_fields(uuid) IS
  'Read-only: lists imported events (event_import_origins) whose six agenda-import columns are all null while observations is non-blank - manual-review candidates for a possible pre-fix data-loss case. Never writes; there is no deterministic way to split free-text observations back into the dedicated columns.';
