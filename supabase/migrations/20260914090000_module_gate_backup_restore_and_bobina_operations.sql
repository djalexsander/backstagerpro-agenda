-- P0 fix (module audit AUDITORIA_MODULOS_OPCIONAIS.md, 2026-09-14): two real
-- backend gaps where a write happened without checking whether the owning
-- optional module is currently active for the company.
--
-- 1) restore_company_backup wrote to ~20 tables spanning nearly every
--    optional module (materiais, estoque, custódias, locações, manutenção,
--    financeiro avançado, RFID, etiquetas/bobina, funcionários, clientes,
--    documentos) without ever checking company_has_active_module for any of
--    them - it called assert_actor_company_operational_access with
--    _feature_key = NULL, which by that function's own design skips the
--    module check entirely (see 20260808100000_enforce_master_tenant_
--    isolation.sql:163-166). A company could restore (or a hand-built
--    payload could inject) data belonging to a module it never purchased.
--
-- 2) The bobina/printer-profile write surface - salvar_perfil_bobina,
--    duplicar_perfil_bobina, excluir_perfil_bobina,
--    definir_perfil_bobina_padrao, salvar_configuracao_impressora (only its
--    finalidade='etiqueta' rows), and the "Company admins write bobina
--    profiles" policy on empresa_bobina_perfis - never checked any module
--    helper, only role (admin_empresa/master_admin) + tenant. Conceptually
--    this belongs to etiquetas_materiais (the table's own comment: "Fonte de
--    dados para o motor de layout" of label printing) but was never wired to
--    entitlement in any layer.
--
-- Fix shape for both: add company_has_active_module('<feature_key>') checks
-- using the SAME helper the rest of the backend already uses for these exact
-- modules (resolve_material_labels_company for etiquetas_materiais,
-- resolve_stock_company/resolve_custody_company/resolve_material_rental_
-- company/resolve_equipment_maintenance_company/resolve_financial_company
-- for the operational-core modules) - no new authorization mechanism is
-- introduced. company_has_active_module has no is_master_admin bypass (see
-- the comment block at the top of 20260808100000_enforce_master_tenant_
-- isolation.sql explaining why can_read/write_company_module lost their own
-- redundant "OR is_master_admin"), so master_admin is already held to the
-- same module-active bar as admin_empresa everywhere this helper is used
-- today; this migration extends that same existing rule to the two gaps
-- above instead of inventing a different rule for them. master_admin's
-- actual current bypass - the admin_empresa role requirement itself - is
-- untouched everywhere in this migration.
--
-- restore_company_backup: a company without a section's module active
-- simply does not get that section restored (the payload variable for that
-- section is nulled out immediately after its own shape validation already
-- confirmed it was a well-formed array, before any tenant-id/FK/dedup check
-- or write runs) - every check for these "optional" collections was already
-- written as "v_x IS NOT NULL AND ...", including the final JSON summary, so
-- nulling the variable is sufficient and requires touching no downstream
-- logic. events/event_days/event_files/financials (the original mandatory
-- collections, deleted-then-reinserted as one atomic core unit together)
-- are deliberately left untouched - splitting that delete-then-reinsert
-- specifically for financials would be a materially bigger, riskier change
-- than this surgical pass, and restore_company_backup already bypasses RLS
-- for every table it touches by virtue of being SECURITY DEFINER, exactly
-- like it does for events. empresa_impressora_config is shared by 3 printer
-- purposes (etiqueta/cupom/documento); only finalidade='etiqueta' rows are
-- dropped when etiquetas_materiais is inactive, never cupom/documento rows.
--
-- Known accepted edge case, not fixed here: financeiro_lancamentos.cliente_id
-- optionally references clientes. If a company has financeiro_avancado
-- active but NOT locacao_materiais (so clientes is skipped) and a
-- lancamento in the payload references a cliente_id, the pre-existing FK
-- validation still raises and aborts the whole restore. This exact failure
-- mode already existed before this migration for any payload that simply
-- omitted the clientes key while including such a lancamento; this migration
-- adds one more legitimate reason a payload can reach that same, unchanged
-- validation. In practice gather_company_backup_data exports every
-- collection unconditionally, so this only manifests when locacao_materiais
-- was deactivated specifically between backup-time and restore-time while
-- financeiro_avancado stayed active. Solving every cross-module payload
-- inconsistency is out of scope for this pass; flagged for awareness.
--
-- No table, column, trigger, policy (other than the one named below) or
-- migration already applied is altered. Only the functions/policy named
-- below are (re)created.
...
DECLARE
  ...
  -- P0 fix (20260914090000): one entitlement flag per module, computed once
  -- and used to null out (or, for empresa_impressora_config, filter) the
  -- payload variables below that belong to a module that is not currently
  -- active for this company. events/event_days/event_files/financials stay
  -- untouched - they are the original, unconditional core collections.
  v_has_gestao_materiais boolean;
  v_has_controle_estoque boolean;
  v_has_checkin_checkout boolean;
  v_has_locacao_materiais boolean;
  v_has_manutencao_equipamentos boolean;
  v_has_financeiro_avancado boolean;
  v_has_checklist_tecnico boolean;
  v_has_painel_operacional boolean;
  v_has_rfid_materiais boolean;
  v_has_etiquetas_materiais boolean;
  v_has_documentos_avancados boolean;
  v_has_funcionarios_access boolean;
BEGIN
  ...
  -- P0 fix (20260914090000): resolve each optional collection's module
  -- entitlement once, up front. company_has_active_module already walks
  -- module_dependencies internally (e.g. controle_estoque requires
  -- gestao_materiais, locacao_materiais requires checkin_checkout, etc. -
  -- see 20260730073000_materials_inventory_stage_one.sql), so checking a
  -- section's own leaf feature_key below is sufficient; no need to AND
  -- multiple keys per section.
  --
  -- No special case for master_admin here, intentionally: company_has_
  -- active_module has never had one - can_read_company_module and
  -- can_write_company_module lost their own "OR is_master_admin" specifically
  -- because it duplicated a bypass (see the comment block at the top of
  -- 20260808100000_enforce_master_tenant_isolation.sql: "can_read_company_
  -- module/can_write_company_module perdem seu próprio OR is_master_admin
  -- porque já delegam para can_read_company_data/can_write_company_data").
  -- A master_admin operating their own linked company is already held to the
  -- same module-active bar as admin_empresa everywhere else this helper is
  -- used (can_write_company_module, user_has_module_action, resolve_stock_
  -- company, resolve_material_labels_company, ...) - this migration extends
  -- that same, already-existing rule to restore instead of introducing a new
  -- one. master_admin's actual current bypass (skipping the admin_empresa
  -- role requirement itself) lives entirely in assert_actor_company_
  -- operational_access above and is untouched by this migration.
  v_has_gestao_materiais := public.company_has_active_module(v_company_id, 'gestao_materiais');
  v_has_controle_estoque := public.company_has_active_module(v_company_id, 'controle_estoque');
  v_has_checkin_checkout := public.company_has_active_module(v_company_id, 'checkin_checkout');
  v_has_locacao_materiais := public.company_has_active_module(v_company_id, 'locacao_materiais');
  v_has_manutencao_equipamentos := public.company_has_active_module(v_company_id, 'manutencao_equipamentos');
  v_has_financeiro_avancado := public.company_has_active_module(v_company_id, 'financeiro_avancado');
  v_has_checklist_tecnico := public.company_has_active_module(v_company_id, 'checklist_tecnico');
  v_has_painel_operacional := public.company_has_active_module(v_company_id, 'painel_operacional');
  v_has_rfid_materiais := public.company_has_active_module(v_company_id, 'rfid_materiais');
  v_has_etiquetas_materiais := public.company_has_active_module(v_company_id, 'etiquetas_materiais');
  v_has_documentos_avancados := public.company_has_active_module(v_company_id, 'documentos_avancados');
  -- funcionarios/event_funcionarios are shared by 3 modules, any one of
  -- which grants access - mirrors "Licensed tenant users read employees" /
  -- "...insert employees" in 20260730043000_enforce_backend_entitlements.sql.
  v_has_funcionarios_access := v_has_financeiro_avancado OR v_has_checklist_tecnico OR v_has_painel_operacional;
  ...
  -- P0 fix (20260914090000): a company without the owning module active does
  -- not get that section restored - treated exactly like an absent payload
  -- key, since every check below (and the JSON summary at the end of this
  -- function) is already written as "v_x IS NOT NULL AND ...".
  IF NOT v_has_locacao_materiais THEN
    v_clientes := NULL;
  END IF;
  IF NOT v_has_funcionarios_access THEN
    v_funcionarios := NULL;
    v_event_funcionarios := NULL;
  END IF;
  IF NOT v_has_checklist_tecnico THEN
    v_event_checklist_items := NULL;
  END IF;
  IF NOT v_has_documentos_avancados THEN
    v_document_templates := NULL;
    v_generated_documents := NULL;
  END IF;
  ...
  -- P0 fix (20260914090000): same rule as above, extended to the
  -- operational-core collections.
  IF NOT v_has_gestao_materiais THEN
    v_categorias := NULL;
    v_materiais := NULL;
  END IF;
  IF NOT v_has_controle_estoque THEN
    v_estoque_localizacoes := NULL;
    v_estoque_saldos := NULL;
    v_estoque_movimentacoes := NULL;
  END IF;
  IF NOT v_has_checkin_checkout THEN
    v_material_custodias := NULL;
    v_material_custodia_eventos := NULL;
  END IF;
  IF NOT v_has_locacao_materiais THEN
    v_material_locacoes := NULL;
    v_material_locacao_itens := NULL;
    v_material_locacao_eventos := NULL;
  END IF;
  IF NOT v_has_manutencao_equipamentos THEN
    v_manutencao_ordens := NULL;
    v_manutencao_ordem_insumos := NULL;
    v_manutencao_ordem_eventos := NULL;
  END IF;
  IF NOT v_has_financeiro_avancado THEN
    v_financeiro_lancamentos := NULL;
    v_financeiro_parcelas := NULL;
    v_financeiro_recebimentos := NULL;
  END IF;
  ...
  -- P0 fix (20260914090000): RFID and label-printing/bobina sections require
  -- their module.
  IF NOT v_has_rfid_materiais THEN
    v_rfid_tags := NULL;
    v_rfid_read_sessions := NULL;
  END IF;
  IF NOT v_has_etiquetas_materiais THEN
    v_etiqueta_modelos := NULL;
    v_etiqueta_impressoes := NULL;
    v_etiqueta_solicitacoes := NULL;
    v_etiqueta_solicitacao_itens := NULL;
    v_empresa_bobina_perfis := NULL;
    -- empresa_impressora_config is shared by 3 printer purposes
    -- (etiqueta/cupom/documento - see salvar_configuracao_impressora's
    -- _finalidade CHECK in 20260811140000_bobina_label_profiles.sql). Only
    -- the label-printing rows belong to etiquetas_materiais, so drop just
    -- those elements instead of the whole collection - dropping the whole
    -- thing would also silently discard a company's unrelated receipt/
    -- document printer configuration just because they do not have the
    -- labels addon. Shape (array-of-objects) was already confirmed above,
    -- so jsonb_array_elements here is safe.
    IF v_empresa_impressora_config IS NOT NULL THEN
      SELECT COALESCE(jsonb_agg(elem), '[]'::jsonb)
      INTO v_empresa_impressora_config
      FROM jsonb_array_elements(v_empresa_impressora_config) AS elem
      WHERE elem ->> 'finalidade' IS DISTINCT FROM 'etiqueta';
    END IF;
  END IF;
  ...
  -- P1-10 follow-up gap: the 6 agenda-import columns
  -- (20260827120000_events_import_fields_and_days.sql) were already
  -- captured into the backup JSON by gather_company_backup_data
  -- (to_jsonb(t) picks up every current column automatically), but this
  -- INSERT's explicit column list predates that migration and silently
  -- dropped them on restore. A backup taken before 20260827120000 simply
  -- has no such keys in its 'eventos' JSON - jsonb_populate_recordset
  -- leaves the matching output columns NULL for those rows (all six are
  -- nullable), so old backups keep restoring exactly as before.
  ...
  SELECT
    ...
    event.state, event.setup_time, event.staff_notes,
...
BEGIN
  ...
  END IF;
  -- P0 fix (20260914090000): bobina profiles belong to etiquetas_materiais.
  IF NOT public.company_has_active_module(v_company_id, 'etiquetas_materiais') THEN
    RAISE EXCEPTION USING ERRCODE = 'LB009', MESSAGE = 'O módulo Etiquetas e Impressão não está ativo para esta empresa.';
  END IF;
...
BEGIN
  ...
  IF v_company_id IS NULL OR (_empresa_id IS NOT NULL AND _empresa_id <> v_company_id) THEN
    RAISE EXCEPTION USING ERRCODE = '42501', MESSAGE = 'Empresa inválida.';
  END IF;
  -- P0 fix (20260914090000): only the label-printing purpose belongs to
  -- etiquetas_materiais - cupom/documento are core printer configuration
  -- shared by every company and must stay unaffected by this module.
  IF _finalidade = 'etiqueta' AND NOT public.company_has_active_module(v_company_id, 'etiquetas_materiais') THEN
    RAISE EXCEPTION USING ERRCODE = 'LB009', MESSAGE = 'O módulo Etiquetas e Impressão não está ativo para esta empresa.';
  END IF;
...
-- P0 fix (20260914090000): empresa_bobina_perfis is 100% dedicated to label
-- printing (its own table comment: "Fonte de dados para o motor de layout em
-- src/lib/label-layout-engine.ts") - unlike empresa_impressora_config, there
-- is no cupom/documento split to preserve here, so the whole write policy is
-- gated. The read policy ("Company members read bobina profiles") is
-- intentionally left untouched - out of scope for this P0 pass.
--
-- Uses can_write_company_module(empresa_id, 'etiquetas_materiais') - not
-- can_write_company_data(...) AND company_has_active_module(...) written out
-- separately, even though that is the exact same boolean logic
-- (can_write_company_module's own body is precisely that AND) - because
-- company_has_active_module is REVOKEd from everyone except its owner
-- (`{postgres=X/postgres}`, confirmed against pg_proc.proacl locally) and is
-- only ever meant to be reached already-nested inside another SECURITY
-- DEFINER function. A function referenced directly in an RLS USING/WITH
-- CHECK clause is privilege-checked against the calling role itself
-- (authenticated), not against the function that owns the policy's table or
-- any SECURITY DEFINER context - the same gotcha already documented for
-- event_belongs_to_company in 20260808090000_fix_event_belongs_to_company_
-- grants.sql. can_write_company_module is the one function in this pair
-- that already carries GRANT EXECUTE ... TO authenticated (see
-- 20260808100000_enforce_master_tenant_isolation.sql's own v_granted list),
-- exactly because it is the helper this codebase's other module-gated
-- policies call directly (funcionarios, event_checklist_items,
-- document_templates, financials all use can_read/write_company_module in
-- their policies, never company_has_active_module itself).
DROP POLICY IF EXISTS "Company admins write bobina profiles" ON public.empresa_bobina_perfis;
CREATE POLICY "Company admins write bobina profiles"
ON public.empresa_bobina_perfis FOR ALL TO authenticated
USING (public.can_write_company_module(empresa_id, 'etiquetas_materiais'))
WITH CHECK (public.can_write_company_module(empresa_id, 'etiquetas_materiais'));
-- ============================================================================
...
DECLARE
  ...
  v_granted regprocedure[] := ARRAY[
    'public.restore_company_backup(uuid,jsonb)',
    'public.salvar_perfil_bobina(text,numeric,numeric,integer,numeric,numeric,numeric,numeric,numeric,numeric,text,numeric,numeric,numeric,text,integer,boolean,uuid,timestamptz,uuid)',
    'public.duplicar_perfil_bobina(uuid,text,uuid)',
    'public.excluir_perfil_bobina(uuid,uuid)',
    'public.definir_perfil_bobina_padrao(uuid,uuid)',
    'public.salvar_configuracao_impressora(text,text,text,numeric,numeric,text,boolean,jsonb,uuid,uuid)'
  ]::regprocedure[];
  v_module_checked regprocedure[] := ARRAY[
    'public.salvar_perfil_bobina(text,numeric,numeric,integer,numeric,numeric,numeric,numeric,numeric,numeric,text,numeric,numeric,numeric,text,integer,boolean,uuid,timestamptz,uuid)',
    'public.duplicar_perfil_bobina(uuid,text,uuid)',
    'public.excluir_perfil_bobina(uuid,uuid)',
    'public.definir_perfil_bobina_padrao(uuid,uuid)',
...
BEGIN
  -- A) CREATE OR REPLACE must not have dropped EXECUTE for authenticated, and
  -- must not have granted it to anon.
  FOREACH v_fn IN ARRAY v_granted LOOP
    IF NOT has_function_privilege('authenticated', v_fn, 'EXECUTE') THEN
      RAISE EXCEPTION USING ERRCODE = 'FN150',
        MESSAGE = format('Correcao quebrou RPC publica: %s perdeu EXECUTE de authenticated.', v_fn);
    END IF;
    IF has_function_privilege('anon', v_fn, 'EXECUTE') THEN
      RAISE EXCEPTION USING ERRCODE = 'FN151',
        MESSAGE = format('%s ficou executavel por anon.', v_fn);
    END IF;
    RAISE NOTICE 'RPC INTACTA | % | authenticated=sim anon=nao', v_fn;
  END LOOP;

  -- B) Every function this migration touched must now literally reference
  -- company_has_active_module in its body (source-level regression guard,
  -- same technique as 20260808100000_enforce_master_tenant_isolation.sql).
  FOREACH v_fn IN ARRAY v_module_checked LOOP
    SELECT pg_get_functiondef(v_fn) INTO v_source;
    IF v_source !~* 'company_has_active_module' THEN
      RAISE EXCEPTION USING ERRCODE = 'FN152',
        MESSAGE = format('%s nao contem mais a checagem de company_has_active_module.', v_fn);
    END IF;
    RAISE NOTICE 'MODULO PROTEGIDO | %', v_fn;
  END LOOP;

  -- C) salvar_configuracao_impressora must gate only finalidade='etiqueta',
  -- never cupom/documento - the literal string 'etiqueta' must appear right
  -- next to the module check so a future edit cannot silently widen it to
  -- every finalidade without this guard noticing.
  SELECT pg_get_functiondef('public.salvar_configuracao_impressora(text,text,text,numeric,numeric,text,boolean,jsonb,uuid,uuid)'::regprocedure)
    INTO v_source;
  IF v_source !~* '_finalidade\s*=\s*''etiqueta''\s+AND\s+NOT\s+public\.company_has_active_module' THEN
    RAISE EXCEPTION USING ERRCODE = 'FN153',
      MESSAGE = 'salvar_configuracao_impressora: a checagem de modulo nao esta restrita a finalidade=etiqueta.';
  END IF;
  RAISE NOTICE 'OK: salvar_configuracao_impressora so exige etiquetas_materiais para finalidade=etiqueta.';

  -- D) empresa_bobina_perfis write policy must now require the module too -
  -- via can_write_company_module, not company_has_active_module directly
  -- (see the policy's own comment: the latter is REVOKEd from authenticated
  -- and would raise "permission denied for function company_has_active_
  -- module" the moment a real RLS check needed it, not the intended
  -- row-security denial).
  IF NOT EXISTS (
    SELECT 1
    FROM pg_catalog.pg_policies
    WHERE schemaname = 'public'
      AND tablename = 'empresa_bobina_perfis'
      AND policyname = 'Company admins write bobina profiles'
      AND qual ILIKE '%can_write_company_module%'
      AND with_check ILIKE '%can_write_company_module%'
  ) THEN
    RAISE EXCEPTION USING ERRCODE = 'FN154',
      MESSAGE = 'Policy de escrita de empresa_bobina_perfis nao exige mais o modulo etiquetas_materiais.';
  END IF;
  RAISE NOTICE 'OK: policy de escrita de empresa_bobina_perfis exige etiquetas_materiais.';

  -- E) restore_company_backup must still delete/insert events unconditionally
  -- (the core collections were deliberately left untouched by this
  -- migration) while also now nulling out every module-gated collection.
  SELECT pg_get_functiondef('public.restore_company_backup(uuid,jsonb)'::regprocedure) INTO v_source;
  IF v_source !~* 'DELETE FROM public\.events WHERE empresa_id = v_company_id' THEN
    RAISE EXCEPTION USING ERRCODE = 'FN155',
      MESSAGE = 'restore_company_backup: o delete/insert incondicional de events sumiu - regressao inesperada.';
  END IF;
  IF v_source !~* 'v_has_gestao_materiais\s*:=\s*public\.company_has_active_module'
     OR v_source !~* 'v_has_etiquetas_materiais\s*:=\s*public\.company_has_active_module'
  THEN
    RAISE EXCEPTION USING ERRCODE = 'FN156',
      MESSAGE = 'restore_company_backup: flags de entitlement de modulo nao encontradas.';
  END IF;
  RAISE NOTICE 'OK: restore_company_backup preserva o core (events) e resolve as flags de modulo.';
END;
$$;

COMMENT ON FUNCTION public.restore_company_backup(uuid, jsonb) IS
  'Restores a company backup transactionally. Core collections (events, event_days, event_files, financials) are always restored; every other, module-owned collection (materiais/estoque/custodias/locacoes/manutencao/financeiro avancado/rfid/etiquetas/bobina/impressora-etiqueta/funcionarios/documentos) is silently skipped when its owning module is not currently active for the company (company_has_active_module, no master_admin bypass, same rule as everywhere else in the backend) - P0 fix 20260914090000.';

COMMENT ON FUNCTION public.salvar_perfil_bobina(text,numeric,numeric,integer,numeric,numeric,numeric,numeric,numeric,numeric,text,numeric,numeric,numeric,text,integer,boolean,uuid,timestamptz,uuid) IS
  'Creates or updates a bobina (label media) profile. Requires etiquetas_materiais to be active for the company (P0 fix 20260914090000) in addition to admin_empresa/master_admin + tenant match.';

COMMENT ON FUNCTION public.duplicar_perfil_bobina(uuid,text,uuid) IS
  'Duplicates a bobina profile. Requires etiquetas_materiais to be active for the company (P0 fix 20260914090000) in addition to admin_empresa/master_admin + tenant match.';

COMMENT ON FUNCTION public.excluir_perfil_bobina(uuid,uuid) IS
  'Soft-deletes a bobina profile. Requires etiquetas_materiais to be active for the company (P0 fix 20260914090000) in addition to admin_empresa/master_admin + tenant match.';

COMMENT ON FUNCTION public.definir_perfil_bobina_padrao(uuid,uuid) IS
  'Marks a bobina profile as the company default. Requires etiquetas_materiais to be active for the company (P0 fix 20260914090000) in addition to admin_empresa/master_admin + tenant match.';

COMMENT ON FUNCTION public.salvar_configuracao_impressora(text,text,text,numeric,numeric,text,boolean,jsonb,uuid,uuid) IS
  'Creates or updates a printer configuration for one finalidade (etiqueta/cupom/documento). Only finalidade=etiqueta additionally requires etiquetas_materiais to be active for the company (P0 fix 20260914090000); cupom/documento remain core, ungated.';
