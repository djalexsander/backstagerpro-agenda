-- Corrige um bug real de produção introduzido por
-- 20260824100000_traceability_open_custodies_enrichment.sql (já aplicada
-- no remoto - por isso NÃO é editada aqui, só substituída por uma versão
-- corrigida via CREATE OR REPLACE): duas referências a
-- "custody.quantidade_pendente" tratavam esse valor como se fosse uma
-- coluna física de material_custodias. Não é - a tabela nunca teve essa
-- coluna (CREATE TABLE original em
-- 20260802160000_material_checkin_checkout_stage_three.sql só tem
-- quantidade_retirada/quantidade_devolvida; quantidade_baixada foi
-- adicionada depois, via ALTER TABLE, por
-- 20260817270000_close_lost_damaged_material_custody.sql). O erro em
-- produção, "column custody.quantidade_pendente does not exist", bate
-- exatamente com isso.
--
-- Fórmula canônica confirmada nas migrations existentes (não presumida):
-- listar_custodias_materiais já calcula isso do mesmo jeito, linha a
-- linha idêntica, em 20260823090000_custody_by_event_reference_filter.sql
-- ('quantidade_pendente', custody.quantidade_retirada -
-- custody.quantidade_devolvida - custody.quantidade_baixada) - a mesma
-- fórmula usada aqui. A CHECK constraint
-- material_custodias_quantidades_contabilizadas_range (mesma migration
-- 20260817270000, "quantidade_devolvida + quantidade_baixada <=
-- quantidade_retirada") garante que essa subtração nunca fica negativa.
--
-- Único ponto quebrado: dentro do item de custodias_abertas (a coluna
-- lida diretamente) e na soma usada para quantidade_fora (mesmo SELECT,
-- SUM sobre a coluna inexistente). Corrigidos com a MESMA expressão nos
-- dois lugares, de propósito - evita quantidade_fora divergir da soma dos
-- quantidade_pendente individuais em custodias_abertas. Revisei a função
-- inteira à procura de outras colunas incompatíveis com o schema atual de
-- material_custodias/estoque_saldos/estoque_localizacoes/materiais/
-- events/material_locacao_itens/material_locacoes/clientes/profiles -
-- nenhuma outra encontrada; todo o resto já é código que rodava em
-- produção antes desta migration (o bloco de "situação atual" nunca leu
-- quantidade_pendente) ou usa exatamente os mesmos nomes de coluna que
-- esse código pré-existente já usa com sucesso.
--
-- Assinatura de resumo_situacao_material não muda (_material_id uuid,
-- _empresa_id uuid) - só o corpo -, então CREATE OR REPLACE simples
-- basta. obter_rastreabilidade_material não é redefinida: seu próprio
-- corpo nunca referenciou quantidade_pendente (só embute o retorno
-- inteiro de resumo_situacao_material sob a chave 'resumo'), então não
-- precisa ser tocada para a correção valer.
--
-- Todo o enriquecimento de 20260824100000 é preservado integralmente:
-- custodias_abertas (com todos os campos por item), quantidade_total,
-- quantidade_disponivel, quantidade_fora, Evento, Locação/cliente e todos
-- os campos que já existiam antes dela. Nenhuma tabela, RPC de escrita,
-- Scanner Remoto, Check-in/Check-out ou arquivo TypeScript é tocado.
